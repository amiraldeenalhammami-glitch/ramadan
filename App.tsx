...
import { Clock, Moon, Sun, Calendar, Star, ChevronDown, ChevronUp, LayoutGrid, Download, X, Volume2, Bell, BellOff, BookOpen, Scale, Microscope, Table as TableIcon, MapPin, Zap, ShieldCheck, Utensils, Coffee, Sparkles, Share } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // ظهور خيار التثبيت تلقائياً عند فتح التطبيق لأول مرة (بعد 2 ثانية لضمان التحميل)
    if (!standalone) {
      setTimeout(() => {
    ...
    const handleBeforeInstallPrompt = (e: any) => {
      e.preventDefault();
      setDeferredPrompt(e);
      // إذا تم التقاط الحدث، نتأكد من إظهار النافذة
      setShowInstallOverlay(true);
    };
  ...
  useEffect(() => {
    ...
    if (isAlertEnabled && timeStr !== lastSoundTriggerRef.current) {
      ...
      if (dayData) {
        ...
        if (isPrayerTime && audioUnlockedRef.current) {
  ...
  const handleInstallClick = () => {
    if (deferredPrompt) {
      ...
      // لمستخدمي آيفون نظهر الإرشادات اليدوية لأن النظام لا يدعم التثبيت البرمجي
      setShowInstallOverlay(true);
    } else {
      alert("خيار التثبيت متوفر عبر متصفح كروم أو عبر إضافة التطبيق للشاشة الرئيسية.");
    }
  };
  ...
  const toggleAlert = (e: React.MouseEvent) => {
    ...
    localStorage.setItem('isAlertEnabled', String(nextValue));
    if (nextValue) {
      setAlertDisabledUntil(null);
      localStorage.removeItem('alertDisabledUntil');
    }
  ...
  return (
    <div className="min-h-screen islamic-pattern text-slate-900 pb-24 overflow-x-hidden selection:bg-amber-300 antialiased" onClick={unlockAudio}>
      
      {/* نافذة تثبيت التطبيق التلقائية */}
      {showInstallOverlay && !isStandalone && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border-4 border-amber-400 relative p-8 text-center space-y-6 animate-in slide-in-from-bottom-10">
            <button onClick={() => setShowInstallOverlay(false)} className="absolute top-4 left-4 p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors"><X size={20} /></button>
            <div className="w-24 h-24 bg-gradient-to-br from-amber-400 to-amber-600 rounded-3xl mx-auto flex items-center justify-center shadow-xl ring-4 ring-amber-100">
              <Download size={48} className="text-white animate-bounce" />
            </div>
            <h3 className="text-2xl font-black font-amiri text-slate-900">تثبيت تطبيق الإمساكية الذكي</h3>
            
            {isIOS ? (
              <div className="space-y-4 bg-slate-50 p-6 rounded-3xl border border-slate-200">
                <p className="text-slate-700 font-bold text-base leading-relaxed">لمستخدمي آيفون، يرجى اتباع الخطوات التالية:</p>
                <ol className="text-right text-slate-600 space-y-3 font-bold text-sm" dir="rtl">
                  <li className="flex items-center gap-3"><span className="bg-amber-400 text-slate-900 w-6 h-6 rounded-full flex items-center justify-center shrink-0">1</span> اضغط على زر المشاركة <Share size={18} className="inline mx-1 text-blue-500" /> أسفل المتصفح.</li>
                  <li className="flex items-center gap-3"><span className="bg-amber-400 text-slate-900 w-6 h-6 rounded-full flex items-center justify-center shrink-0">2</span> اختر "إضافة إلى الشاشة الرئيسية".</li>
                  <li className="flex items-center gap-3"><span className="bg-amber-400 text-slate-900 w-6 h-6 rounded-full flex items-center justify-center shrink-0">3</span> اضغط "إضافة" في الزاوية العلوية.</li>
                </ol>
              </div>
            ) : (
              <div className="space-y-4">
                <p className="text-slate-600 font-bold">ثبت التطبيق الآن للوصول السريع للمواقيت حتى بدون إنترنت</p>
                <button 
                  onClick={handleInstallClick} 
                  className="w-full py-5 bg-slate-950 text-amber-400 rounded-2xl font-black text-xl shadow-xl hover:bg-black transition-all transform hover:scale-105 active:scale-95 border-b-4 border-amber-600"
                >
                  تثبيت على جهازي الآن
                </button>
              </div>
            )}
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Ab3ad Academy • PWA Technology</p>
          </div>
        </div>
      )}
              ...
              <div className="mt-4 flex flex-col items-center gap-6">
                ...
                <div className="mt-10 flex flex-col md:flex-row items-center gap-5">
                  ...
                  <button onClick={toggleAlert} className={`flex items-center gap-3 px-10 py-4 rounded-2xl font-black font-amiri text-lg transition-all shadow-lg transform hover:scale-105 active:scale-95 border-b-4 ${isAlertEnabled ? 'bg-red-500 text-white border-red-700' : 'bg-amber-400 text-slate-900 border-amber-600'}`}>
                    ...
                    <span>{isAlertEnabled ? "إيقاف المنبه" : "تفعيل المنبه الصوتي"}</span>
                  </button>
                  
                  {/* زر معاينة الصوت */}
                  <button onClick={previewSound} className="group p-5 bg-white rounded-2xl text-slate-600 hover:text-amber-600 transition-all border-2 border-slate-100 shadow-md flex items-center gap-3 border-b-4 border-slate-200">
                    <div className="bg-slate-50 p-2 rounded-full group-hover:bg-amber-50 transition-colors">
                      <Volume2 size={24} className="group-hover:animate-bounce" />
                    </div>
                    <span className="font-bold text-base font-amiri">اختبار الصوت</span>
                  </button>

                  {/* زر التثبيت في الصفحة */}
                  <button onClick={handleInstallClick} className="flex items-center gap-3 px-10 py-4 bg-slate-100 rounded-2xl font-black font-amiri text-lg text-slate-900 hover:bg-slate-200 transition-all border-b-4 border-slate-300 shadow-lg transform hover:scale-105">
                    <Download size={24} />
            ...
            <div className="p-8 md:p-12 space-y-12 border-t border-slate-100 bg-slate-50/20">
              ...
              <div className="prose prose-slate max-w-none font-amiri text-lg md:text-xl text-slate-950 leading-relaxed space-y-8">
                <p className="p-6 bg-white rounded-3xl border border-slate-200 shadow-sm text-slate-950 font-bold">
                  تعتمد هذه الإمساكية بشكل حصري على <span className="text-amber-700 font-black underline decoration-amber-400">التقويم الهاشمي السوري</span>، وهو المرجع الزمني الأدق والأكثر عراقة في الجمهورية العربية السورية.
                </p>
              ...
              {/* الجدول الكامل لمقارنة المعايير الحسابية العالمية */}
                ...
                <div className="overflow-x-auto rounded-3xl border border-slate-400 bg-white shadow-xl">
                  <table className="w-full text-right font-amiri text-lg md:text-xl border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-400">
                      ...
                      <tr className="bg-slate-50">
                        <td className="p-6 font-bold text-slate-950 border-l border-slate-200">المعهد الفلكي بجامعة طهران</td>
                        <td className="p-6 text-center font-mono text-slate-950 font-black border-l border-slate-200" dir="ltr">17.7°</td>
                        <td className="p-6 text-center font-mono text-slate-950 font-black border-l border-slate-200" dir="ltr">14.0°</td>
                        <td className="p-6 text-slate-950 font-bold">زاوية منخفضة للفجر</td>
                      </tr>
                ...
                <div className="space-y-8">
                  <div className="flex gap-5">
                    ...
                    <div className="text-slate-950">
                      ...
                      <p className="text-slate-900 font-amiri leading-relaxed text-lg">اعتُمد وقت الإمساك قبل أذان الفجر بـ 15 دقيقة احتياطاً عملاً بالهدي النبوي الشريف لضمان براءة الذمة.</p>
                    </div>
                  </div>
                </div>
              </div>
