...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // التحقق من حالة التطبيق (هل هو مثبت حالياً؟)
    ...
    const handleBeforeInstallPrompt = (e: any) => {
      e.preventDefault();
      setDeferredPrompt(e);
      // إظهار نافذة التثبيت تلقائياً بعد 3 ثوانٍ إذا لم يكن التطبيق مثبتاً
      if (!isAppStandalone) {
    ...
    // حدث يتم إطلاقه عند نجاح التثبيت
  ...
  const handleInstallClick = async () => {
    if (!deferredPrompt) {
      if (isIOS) {
        alert("لمستخدمي آيفون: اضغط على زر 'مشاركة' (Share) في المتصفح ثم اختر 'إضافة إلى الشاشة الرئيسية' (Add to Home Screen).");
      } else {
        alert("التطبيق جاهز للتثبيت من خلال إعدادات المتصفح.");
      }
      return;
    }
    ...
    <div className="min-h-screen islamic-pattern text-slate-900 pb-24 overflow-x-hidden selection:bg-amber-300 antialiased" onClick={() => audioRef.current?.play().then(() => {audioRef.current?.pause(); audioRef.current!.currentTime=0;}).catch(()=>{})}>
      ...
      {showInstallOverlay && !isStandalone && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border-4 border-amber-400 relative overflow-hidden">
            ...
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-600 font-bold text-sm leading-relaxed px-4">
                  {isIOS 
                    ? "لمستخدمي آيفون: اضغط على زر 'مشاركة' في المتصفح ثم اختر 'إضافة إلى الشاشة الرئيسية'." 
                    : "احصل على وصول سريع للمواقيت وتنبيهات الأذان حتى بدون اتصال بالإنترنت."}
                </p>
              </div>
              {!isIOS && (
                <button onClick={handleInstallClick} className="w-full py-4 bg-slate-900 text-amber-400 rounded-2xl font-black text-xl shadow-xl hover:bg-black transition-all">تثبيت الآن</button>
              )}
            </div>
          </div>
        </div>
        ...
        {appStatus !== 'POST_RAMADAN' && (
          <section className="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl p-6 md:p-10 text-center border-b-[8px] border-[#fcc52c] relative">
            <div className="flex flex-col items-center">
              ...
              {/* زر التثبيت الإضافي في الواجهة الرئيسية */}
              {!isStandalone && (
                <button onClick={handleInstallClick} className="mt-6 flex items-center gap-2 text-slate-500 hover:text-amber-600 font-bold transition-colors">
                  <Download size={18} />
                  <span className="text-sm font-amiri">تنزيل التطبيق على جهازك</span>
                </button>
              )}
            </div>
          </section>
