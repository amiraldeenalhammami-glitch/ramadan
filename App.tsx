...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // التحقق من حالة المنصة والتثبيت
    ...
    // التقاط حدث التثبيت للمتصفحات (Android/Windows)
    const handleBeforeInstallPrompt = (e: any) => {
      e.preventDefault();
      setDeferredPrompt(e);
      // إظهار التنبيه فوراً إذا لم يكن مثبتاً
      if (!isAppStandalone) {
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      // للأنظمة التي تدعم التثبيت المباشر (Android, Chrome/Edge on Desktop)
      ...
      // لآيفون: لا يوجد API مباشر، يجب إرشاد المستخدم
      alert("تثبيت على آيفون:\n1. اضغط على زر 'مشاركة' (Share) في أسفل المتصفح.\n2. اختر 'إضافة إلى الشاشة الرئيسية' (Add to Home Screen).\n3. اضغط 'إضافة' (Add) في الزاوية العلوية.");
    } else {
      // إذا لم يطلق المتصفح الحدث بعد، نوجهه للقائمة الجانبية
      alert("التطبيق جاهز للعمل أوفلاين. يمكنك تثبيته بالضغط على الثلاث نقاط في زاوية المتصفح ثم اختيار 'تثبيت التطبيق' أو 'Install App'.");
    }
  };
      ...
      {showInstallOverlay && !isStandalone && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border-4 border-[#fcc52c] relative overflow-hidden">
            <button onClick={() => setShowInstallOverlay(false)} className="absolute top-4 left-4 p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors z-10"><X size={20} /></button>
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-600 font-bold text-base leading-relaxed px-4">
                  {isIOS 
                    ? "اضغط على زر المشاركة ثم 'إضافة إلى الشاشة الرئيسية' ليعمل التطبيق بدون إنترنت." 
                    : "احصل على وصول فوري للمواقيت وتنبيهات الأذان بضغطة زر واحدة حتى بدون اتصال."}
                </p>
              </div>
              <button 
              ...
              >
                {isIOS ? "كيفية التثبيت؟" : "تثبيت كـ تطبيق ذكي"}
              </button>
            </div>
          </div>
        </div>
        ...
        {appStatus !== 'POST_RAMADAN' && (
          <section className="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl p-6 md:p-10 text-center border-b-[8px] border-[#fcc52c] relative">
            <div className="flex flex-col items-center">
              ...
              {!isStandalone && (
                <button onClick={handleInstallClick} className="mt-8 flex items-center gap-3 bg-slate-100 px-8 py-3 rounded-full text-slate-700 hover:bg-[#fcc52c] hover:text-black font-black transition-all shadow-sm border border-slate-200">
                  <Download size={20} />
                  <span className="text-sm font-amiri uppercase tracking-widest">تثبيت التطبيق على جهازك</span>
                </button>
              )}
            </div>
          </section>
