...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Check if install prompt already captured in window
    if ((window as any).deferredInstallPrompt && !isAppStandalone) {
    ...
    // Also trigger for iOS after a small delay if not standalone
    if (isIOSDevice && !isAppStandalone && !sessionStorage.getItem('installPromptDismissed')) {
      setTimeout(() => setShowInstallOverlay(true), 2000);
    }
  ...
  return (
    <div className="min-h-screen islamic-pattern text-slate-900 pb-24 overflow-x-hidden selection:bg-amber-300 antialiased" onClick={() => { if(audioRef.current) { audioRef.current.play().then(() => {audioRef.current?.pause(); audioRef.current!.currentTime=0;}).catch(()=>{})} }}>
      ...
      {/* نافذة التثبيت الذكية - تم تحديثها لتشمل "إجراء ذلك لاحقاً" */}
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border-4 border-[#fcc52c] relative overflow-hidden">
            ...
            <div className="p-8 text-center space-y-6">
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  {isIOS ? "كيفية التثبيت؟" : "تنزيل التطبيق"}
                </button>
                <button 
            ...
            <div className="flex flex-col items-center">
              ...
              {!isStandalone && (
                <button onClick={handleInstallClick} className="mt-8 flex items-center gap-3 bg-slate-900 px-10 py-4 rounded-2xl text-[#fcc52c] hover:bg-black font-black transition-all shadow-xl border-b-4 border-amber-600 transform hover:scale-105">
                  <Download size={24} />
                  <span className="text-lg font-amiri uppercase tracking-widest">تنزيل التطبيق الآن</span>
                </button>
              )}
            </div>
          </section>
